...
def test_convert_to_gds(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/convert_to_gds/config")
        data_path = Path(".tests/unit/convert_to_gds/data")
        expected_path = Path(".tests/unit/convert_to_gds/expected")
