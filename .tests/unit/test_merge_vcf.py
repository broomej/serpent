...
def test_merge_vcf(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/merge_vcf/config")
        data_path = Path(".tests/unit/merge_vcf/data")
        expected_path = Path(".tests/unit/merge_vcf/expected")
