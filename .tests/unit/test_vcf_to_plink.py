...
def test_vcf_to_plink(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/vcf_to_plink/config")
        data_path = Path(".tests/unit/vcf_to_plink/data")
        expected_path = Path(".tests/unit/vcf_to_plink/expected")
