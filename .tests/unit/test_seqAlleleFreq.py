...
def test_seqAlleleFreq(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/seqAlleleFreq/config")
        data_path = Path(".tests/unit/seqAlleleFreq/data")
        expected_path = Path(".tests/unit/seqAlleleFreq/expected")
