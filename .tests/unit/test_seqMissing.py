...
def test_seqMissing(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/seqMissing/config")
        data_path = Path(".tests/unit/seqMissing/data")
        expected_path = Path(".tests/unit/seqMissing/expected")
